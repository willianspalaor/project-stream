...
function _setCurrentData(){data.client_anime.current_season=currentSeason.season;data.client_anime.current_episode=currentEpisode.episode;data.client_anime.last_activity=new Date().toLocaleString();_saveClientAnime(data.client_anime)}
...
checkVideo();function checkVideo(){_checkUrlVideo(video.url,function(is_working,changeStatus=!0){if(is_working){callback(video);found=!0;if(changeStatus){_saveVideoStatus(videos_not_working)}}else{Player_Helper.clearConsole();if(!found){videos_not_working.push(video);video=getVideo(index);index++;if(video){checkVideo()}}}})}}
...
function _saveClientAnime(dataClient,callback){$.ajax({type:'POST',url:'/anime/saveClientAnime',data:dataClient,success:function(response){if(typeof(callback)==='function'){callback(JSON.parse(response))}},error:function(xhr,ajaxOptions,thrownError){console.log(xhr.status);console.log(thrownError)}})}
...
function _checkUrlVideo(url,callback){let xhr=new XMLHttpRequest();xhr.open("GET",url,!0);xhr.onreadystatechange=function(oEvent){if(xhr.status===200){callback(!0);xhr.abort()}else{callback(!1)}};xhr.addEventListener("error",function(){callback(!0,!1)});xhr.send(null)}
